...
#--------------------------------------
@app.route('/milestone5',methods=['GET','POST'])  
def home5():
    import pandas as pd
    import psycopg2
    ...
    if request.method == 'POST':
        ...
        finished_sqft = int(request.form['finished_sqft'])
        lot_size_sqft = int(request.form['lot_size_sqft'])
        ...
        filtered_homes = filtered_homes[abs(filtered_homes['finished_sqft'] - finished_sqft) <= 5]
        filtered_homes = filtered_homes[abs(filtered_homes['lot_size_sqft'] - lot_size_sqft) <= 5]
        ...
        if total_similarity == 0:
            estimated_price = "No comparable homes found."
        else:
        ...
        return render_template('price.html', estimated_price=estimated_price)
    else:
        return render_template('price.html', estimated_price=None)
